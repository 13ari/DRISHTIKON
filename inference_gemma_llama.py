...
MODEL_NAME = "Llama-4-Scout-17B-16E-Instruct"  # Updated model name
HEADERS = {
...
TRANSLATION_CACHE = {}
CACHE_FILE = "prompt_translation_cache_llama_api.json"
...
def setup_logger(language, prefix):
    ...
    log_file = os.path.join(LOG_DIR, f"{language}_{prefix}_llama_api_{timestamp}.log")
...
def process_column(qcol: str, prefix: str, df: pd.DataFrame, infer_fn, language: str) -> None:
    output_csv = f"{language}_{prefix}_output_with_answers_llama_api.csv"
    checkpoint_file = f"checkpoint_{language}_{prefix}_llama_api.json"  # Using JSON instead of pkl
    df_out = df.copy()
    df_out["ChosenOption"] = ""
    start_idx = 0
...
def combine_outputs():
    ...
    for lang in CSV_PATHS.keys():
        ...
        for prefix in list(QUESTION_COLUMNS.values()) + ["cc"]:
            csv_file = f"{lang}_{prefix}_output_with_answers_llama_api.csv"
    ...
    for lang, df in combined_dfs.items():
        combined_file = f"{lang}_combined_llama_api_results.csv"
    ...
    for lang, csv_path in CSV_PATHS.items():
        ...
        try:
            ...
            for p in list(QUESTION_COLUMNS.values()) + ["cc"]:
                print(f"- {lang}_{p}_output_with_answers_llama_api.csv")
