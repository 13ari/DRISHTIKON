...
# API Settings for Gemini (for translations)
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY')
GEMINI_MODEL = 'gemini-2.0-flash'  # or other appropriate model

# Cache for storing translated prompts
TRANSLATION_CACHE = {}
CACHE_FILE = "prompt_translation_cache_phi4_multimodal.json"

# Checkpoint cache for storing results
CHECKPOINT_DIR = "phi4_checkpoints"
...
def setup_logger(language, prefix):
    ...
    log_file = os.path.join(LOG_DIR, f"{language}_{prefix}_phi4_multimodal_{timestamp}.log")
...
# Quantization config for 4-bit quantization
...
# Standard inference using Phi-4 multimodal model with translation
def infer_phi4_standard(question: str, options: List[str], image_path: str, language: str = "English", logger=None) -> str:
    prompt = (
        f"Question: {question}\n"
        "Options:\n"
        f"1. {options[0]}\n"
    ...
    # Process for Phi-4 multimodal model
...
# Cultural-dimension inference using Phi-4 multimodal model with translated prompt
def infer_phi4_cultural(question: str, options: List[str], image_path: str, language: str = "English", logger=None) -> str:
...
def process_column(qcol: str, prefix: str, df: pd.DataFrame, infer_fn, language: str) -> None:
    output_csv = f"{language}_{prefix}_output_with_answers_phi4_multimodal.csv"
    checkpoint_file = f"checkpoint_{language}_{prefix}_phi4_multimodal.json"  # Using JSON instead of pkl
    df_out = df.copy()
    df_out["ChosenOption"] = ""
    start_idx = 0
...
def combine_outputs():
    ...
    for lang in CSV_PATHS.keys():
        ...
        for prefix in list(QUESTION_COLUMNS.values()) + ["cc"]:
            csv_file = f"{lang}_{prefix}_output_with_answers_phi4_multimodal.csv"
    ...
    for lang, df in combined_dfs.items():
        combined_file = f"{lang}_combined_phi4_multimodal_results.csv"
...
if __name__ == "__main__":
    print(f"Starting inference with {MODEL_PATH}")
    ...
    for lang, csv_path in CSV_PATHS.items():
        ...
        try:
            ...
            for qcol, prefix in QUESTION_COLUMNS.items():
                ...
                # standard inference
                process_column(qcol, prefix, df, infer_phi4_standard, lang)
                # cultural only for 'r'
                if prefix == "r":
                    process_column(qcol, "cc", df, infer_phi4_cultural, lang)

            print(f"\nGenerated CSV files for {lang}:")
            for p in list(QUESTION_COLUMNS.values()) + ["cc"]:
                print(f"- {lang}_{p}_output_with_answers_phi4_multimodal.csv")
