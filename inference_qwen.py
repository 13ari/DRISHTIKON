...
TRANSLATION_CACHE = {}
CACHE_FILE = "prompt_translation_cache_qwenvl.json"
...
def setup_logger(language, prefix):
    ...
    log_file = os.path.join(LOG_DIR, f"{language}_{prefix}_qwenvl_{timestamp}.log")
...
def process_column(qcol: str, prefix: str, df: pd.DataFrame, infer_fn, language: str) -> None:
    output_csv = f"{language}_{prefix}_output_with_answers_qwenvl.csv"
    checkpoint  = f"checkpoint_{language}_{prefix}_qwenvl.pkl"
    df_out = df.copy()
    df_out["ChosenOption"] = ""
    start_idx = 0
    ...
    for lang, csv_path in CSV_PATHS.items():
        ...
        try:
            ...
            for p in list(QUESTION_COLUMNS.values()) + ["cc"]:
                print(f"- {lang}_{p}_output_with_answers_qwenvl.csv")
